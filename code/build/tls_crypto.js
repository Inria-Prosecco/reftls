...
const nonce = function (n) {
			const buf = crypto.randomBytes(n);
			return buf.toString('hex');
};

const aes_gcm_encrypt = function (k, n, p, ad) {
...
const aes_gcm_decrypt = function (k, n, c, ad) {
...
const random12Bytes = function (a) {
			return randomBytes(12);
};
const random16Bytes = function (a) {
			return randomBytes(16);
};
const random32Bytes = function (a) {
...
const sha256_rsa = function (data) {
...
const hkdf_expand_label = function (secret, label, hashvalue, len) {
			var count = 0;
			var currlen = 0;
			var prev = "";
			var t = "";
			...
			console.log("EXPAND secret:" + secret + "\nlabel:" + label + "\nhash:" + hashvalue + "\ninfo:" + info + "\nresult(b):" + res);
			return res;
};

var deriveKeys_gcm_12 = function (ms, ctx) {
			let kb = tls12_prf_label(ms, 'key expansion', ctx, 40);
...
var deriveKeys_cbc_10 = function (ms, ctx) {
			let kb = tls12_prf_label(ms, 'key expansion', ctx, 104);
...
var deriveKeys_cbc_12 = function (ms, ctx) {
			let kb = tls12_prf_label(ms, 'key expansion', ctx, 72);
			let sp1 = util.split(kb, 20);
			let sp2 = util.split(sp1.snd, 20);
			let sp3 = util.split(sp2.snd, 16);
			return {
...
var deriveKeys_gcm_13 = function (ck, sk) {
			return {
...
var aes_gcm_encrypt_13 = function (keys, plain, ct) {
...
var aes_gcm_decrypt_13 = function (keys, cipher) {
			...
			if (frag.auth_ok === true) return { valid: true, plaintext: sp.fst, ct: sp.snd };else return { valid: false, plaintext: sp.fst, ct: sp.snd };
};
...
var aes_gcm_encrypt_12 = function (keys, plain, ad) {
			...
			let cipher = n8 + aes_gcm_encrypt(keys.writeKey, nonce, plain, snb + ad + lenb);
			keys.writeSn = keys.writeSn + 1;
			return cipher;
};
...
var aes_gcm_decrypt_12 = function (keys, cipher, ad) {
			...
			let adb = snb + ad + lenb;
			let frag = aes_gcm_decrypt(keys.readKey, nonce, cipher, adb);
			keys.readSn = keys.readSn + 1;
			if (frag.auth_ok === true) return { valid: true, plaintext: frag.plaintext };else return { valid: false, plaintext: frag.plaintext };
};
...
var aes_cbc_sha_decrypt_10 = function (keys, cipher, ad) {
			...
			let adb = snb + ad + lenb;
			let m = hmac_sha1(keys.readMacKey, adb + sp2.fst);

			let newIv = cbc_last_block(cipher, 16);
			keys.readSn = keys.readSn + 1;
			keys.readIv = newIv;
			if (m === sp2.snd) return { valid: true, plaintext: sp2.fst };else return { valid: false, plaintext: sp2.fst };
};
...
const BigInteger = {
    ...
    */
			compareTo: function (th, a) {
						...
						if (!!r) return r;
						if (!!s) return s;
						while (--i >= 0) if ((r = x[i & 255] - y[i & 255]) != 0) return r;
						return 0;
			},
    ...
    */
			LshiftTo: function (th, n, r) {
						var a = th.array,
						    ...
						    i = 0;
						for (i = th.t - 1; i >= 0; --i) b[i + ds + 1 & 255] = a[i & 255] >> cbs | c, c = (a[i & 255] & bm) << bs;
    ...
    */
			RshiftTo: function (th, n, r) {
						...
						b[0] = a[ds & 255] >> bs;
						for (i = ds + 1; i < th.t; ++i) b[i - ds - 1 & 255] |= (a[i & 255] & bm) << cbs, b[i - ds & 255] = a[i & 255] >> bs;
    ...
    */
			subTo: function (th, y, r) {
						...
						r.s = c < 0 ? -1 : 0;
						if (c < -1) z[i++ & 255] = this.BI_DV + c;else if (c > 0) z[i++ & 255] = c;
						r.t = i;
						this.clamp(r);
			},
    ...
    */
			multiplyTo: function (th, a, r) {
						...
						r.t = i + y.t;
						while (--i >= 0) v[i & 255] = 0;
						for (i = 0; i < y.t; ++i) v[i + x.t & 255] = this.am(x, 0, w[i & 255], r, i, 0, x.t);
						r.s = 0;
						this.clamp(r);
    ...
    */
			squareTo: function (th, r) {
						...
						for (i = 0; i < x.t - 1; ++i) {
									...
									if ((v[i + x.t & 255] += this.am(x, i + 1, 2 * u[i & 255], r, 2 * i + 1, c, x.t - i - 1)) >= this.BI_DV) v[i + x.t & 255] -= this.BI_DV, v[i + x.t + 1 & 255] = 1;
						}
...
var rsa_pss = {
    ...
    */
			MGF: function (seed, length) {
						...
						for (i = 0; i < n; i++) {
									for (c = '', j = 0; j < 4; j++) c += encoding.b2h(i >> 24 - 8 * j & 255);
    ...
    */
			verify: function (data, signature, pub) {
						...
						for (i = 0; i < s.length; i++) {
									if (i < 2 * (n - hs - 1)) DB += s[i];else if (i < 2 * (n - 1)) sm += s[i];else pad += s[i];
						}
						if (pad != "bc") {
									this.error_code = 7;
									return false;
						...
						this.error_code = 0;
						return encoding.astr2hstr(sha256_rsa("\x00\x00\x00\x00\x00\x00\x00\x00" + m + sm)) == s;
			}
};
