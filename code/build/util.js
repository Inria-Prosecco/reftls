...
const repr_bytes = function (data) {
	...
	if (l < 256) return 1;else if (l < 65536) return 2;else if (l < 16777216) return 3;else return 4;
};
const zeroes = function (l) {
	var v = "";
...
const bytes_of_int = function (n, l) {
	...
	const nl = nb.length;
	const l2 = 2 * l;
	if (nl > l2) throw "bytes_of_int given too short a length" + new Error().stack;else if (nl < l2) return zeroes(l2 - nl) + nb;else return nb;
};
const int_of_bytes = function (nb, l) {
	if (getLength(nb) != l) throw "int_of_bytes given incorrect length" + new Error().stack;else return parseInt(nb, 16);
};

const split = function (data, n) {
	const n2 = 2 * n;
	return {
		fst: data.slice(0, n2),
		snd: data.slice(n2)
...
const getLength = function (d) {
	if (d.length % 2 == 0) return d.length / 2;else throw "Length must be given even-length string, given d.length:" + d.length + ",d.substr(0,10):" + d.substring(0, 10) + new Error().stack;
};
