...
var mapError = function<T,U> (r:result<T>, f:(x:T) => U) : result<U> {
    switch (r.result) {
    case "Correct":
...
var bindError = function<T,U> (r:result<T>, f:(x:T) => result<U>): result<U> {
    switch (r.result) {
    case "Correct":
	return f(r.value)
    case "Error":
...
function Correct<T>(v:T) : result<T> {
    return ({
	result: "Correct",
	value: v,
    })
...
function Incorrect<T>(c:string, d:string) : result<T> {
    return ({
	result: "Error",
	code: c,
	desc: d,
...
function Pair<T,U>(x:T, y:U) : pair<T,U>{
    return ({
	fst: x,
	snd: y
    })
...
function reverse(o: stringmap) : stringmap {
    let no = {}
...
function lookup(t:stringmap, i:string) {
    if (i in t) return t[i]
    else return i
}
const AD_rev = reverse(AD)

function AD_lookup(x:string) {
...
function CS_lookup(x:string) {
    return lookup(CS_rev, x)
}

function KEX(x:string) {
    const c = CS_lookup(x);
    if (c === "TLS_AES_128_GCM_SHA256") return "ECDHE";
    if (c.startsWith("TLS_RSA_")) return "RSA";
    else if (c.startsWith("TLS_DHE_")) return "DHE";
    else if (c.startsWith("TLS_ECDHE_")) return "ECDHE";
    else throw "server chose non-RSA/DHE/ECDHE ciphersuite";
}

function AE(pv:string, x:string) {
    const c = CS_lookup(x);
...
function vlsplit(n:number, data:string) : result<pair<string,string>>{
    //console.log("splitting n:"+n+",d:"+data);
    const sp = util.split(data, n);
    const l = util.int_of_bytes(sp.fst, n);
    if (l <= util.getLength(sp.snd)) return (Correct(util.split(sp.snd, l)));
    else return (Incorrect(AD.decode_error,
			   "vlsplit n:" + n + ",l:" + l + ",d:" + data + "\n" + stackTrace.get()));
}

function vlparse(n:number, data:string) : result<string>{
    //console.log("splitting n:"+n+",d:"+data);
    const sp = util.split(data, n);
    const l = util.int_of_bytes(sp.fst, n);
    if (l == util.getLength(sp.snd)) return (Correct(sp.snd));
    else return (Incorrect(AD.decode_error,
			   "vlparse n:" + n + ",l:" + l + ",d.length" +
			   data.length + "\n" + stackTrace.get().join("\n")));
}
/* HandshakeMessages */
...
function parseMessage(buf:string) : result<pair<?hs_msg,string>> {
    if (util.getLength(buf) < 4) return (Correct(Pair(undefined, buf)))
    else {
...
//    tls13_draft_version: "ff02",
}
const EXT_rev = reverse(EXT);

function EXT_lookup(x:string) {
...
function clientExtensionBytes(ext_type, ext_data) {
	...
	return ext_type + vlbytes(2, vlbytes(2, ext_data.map(d => d.name_type +
							     vlbytes(2, d.host_name)).join("")))
	...
	return ext_type + vlbytes(2, vlbytes(2, ext_data.map(d => vlbytes(1, d))
					     .join("")))
...
function serverExtensionBytes(ext_type, ext_data) {
	...
	return ext_type + vlbytes(2, ext_data.map((d, i) => vlbytes(1, d)).join(
	    ""))
    case EXT.key_share:
	return ext_type + vlbytes(2, ext_data.dh_group +
				  vlbytes(2, ext_data.dh_public))
    case EXT.pre_shared_key:
...
function extensionsBytes(exts, f) {
    ...
    if (util.getLength(res) == 0) return util.empty_bytes
    else return vlbytes(2, res);
}


function parseServerNames(b) : result<[server_name]>{
    return bindError(vlparse(2, b),(snl => {
	const sn = [];
	while (util.getLength(snl) > 0) {
	    const sp1 = util.split(snl, 1);
	    const sp2 = vlsplit(2, sp1.snd);
	    let res:result<pair<string,string>> = sp2;
	    switch (res.result) {
	    case "Error":
...
function parseProtocolNames(b) {
	...
	while (util.getLength(pnl) > 0) {
	    ...
	    let res:result<pair<string,string>> = sp;
	    switch (res.result) {
	    case "Error":
		return Incorrect(res.code,res.desc);
	    case "Correct":
...
function parseNextProtocolNames(pnl) : result<[string]>{
    const pn = [];
    while (util.getLength(pnl) > 0) {
	const sp = vlsplit(1, pnl);
	let res:result<pair<string,string>> = sp;
...
function parseSignatureAlgorithmsList(b) {
    if (b.length % 4 != 0) return Incorrect(AD.decode_error,
					    "parseSignatureAlgorithmsList: not a multiple of 4");
    const sa = [];
...
function parseSupportedGroups(b) {
	...
	let res;
	if (b.length % 4 != 0) res = Incorrect(AD.decode_error,
					       "parseSupportedGroups: not a multiple of 4");
	else {
	    const ec = [];
...
function parseSupportedVersions(b) {
	...
	let res;
	if (b.length % 4 != 0) res = Incorrect(AD.decode_error,
					       "parseSupportedVersions: not a multiple of 4");
	else {
	    const pv = [];
...
function parseKeyShares(b) : result<[key_share]> {
    return bindError(vlparse(2, b),(b => {
	const ks = [];
	let curr = b;
...
function parsePSKIdentities(b) : result<[{psk_identity:string}]>{
//    console.log("PSK Identities:" + b);
	...
	while (util.getLength(curr) > 0) {
		...
		return Incorrect(res.code,res.desc);
	    case "Correct": {
		if (util.getLength(res.value.snd) < 4)
		    return Incorrect(AD.decode_error,"missing obfuscated_ticket_age");
...
function parseDistinguishedNames(dnl) : result<[string]>{
    const dn = [];
...
function parseClientExtension(b) {
    if (util.getLength(b) < 4) return Incorrect(AD.decode_error,
						"clientExtension has less than 4 bytes")
    else {
	...
	return bindError(vlsplit(2, sp.snd),(d => {
	    ...
	    case EXT.next_protocol_negotiation:
		if (util.getLength(d.fst) != 0) return Incorrect(AD.decode_error,
								 "clienthello must contain empty NPN")
		else return Correct(Pair({
		    extension_type: sp.fst,
		    extension_data: []
		}, d.snd))
//	    case EXT.tls13_draft_version:
...
//		}, d.snd))
	    ...
	    case EXT.pre_shared_key:   // TODO: Add PSK Binder
		return mapError(parsePSKIdentities(d.fst),(pfl => Pair({
		    extension_type: sp.fst,
		    extension_data: pfl
		}, d.snd)))
	     default: //TODO: EalyDataIndication
		return Correct(Pair({
		    extension_type: sp.fst,
		    extension_data: d.fst
		}, d.snd))
...
function parseServerExtension(b) {
    if (util.getLength(b) < 4) return Incorrect(AD.decode_error,
						"serverExtension has less than 4 bytes")
    else {
	const sp = util.split(b, 2);
	return bindError(vlsplit(2, sp.snd),(d => {
	    switch (sp.fst) {
		...
		return bindError(parseProtocolNames(d.fst),(pnl => {
		    if (pnl.length > 1) return Incorrect(AD.decode_error,
							 "serverHello alpn extension not singleton");
		    else return Correct(Pair({
			extension_type: sp.fst,
			extension_data: pnl
		    }, d.snd))
	    ...
	    case EXT.server_name:
		if (util.getLength(d.fst) != 0) return Incorrect(AD.decode_error,
								 "serverHello server_name extension not empty")
		else return Correct(Pair({
		    extension_type: sp.fst,
		    extension_data: []
		}, d.snd))
...
function parseExtensions(b, f: (x:string) => result<pair<extension,string>>) {
    if (util.getLength(b) == 0)
	return Correct({});
    else {
	return bindError(vlparse(2, b),(extb => {
		...
		const ce = f(extb);
		let res:result<pair<extension,string>> = ce;
		switch (res.result) {
		case "Correct":
...
function parseCipherSuites(b) : result<string[]>{
    if (b.length % 4 != 0) return Incorrect(AD.decode_error,
					    "parseCipherSuites: not a multiple of 4");
    const cs = [];
...
function parseCompressions(b) {
    if (b.length % 2 != 0) return Incorrect(AD.decode_error,
					    "parseCompressions: not a multiple of 4");
    const cs = [];
...
const fullClientHello = (function () {
    ...
    ch.extensions[EXT.application_layer_protocol_negotiation] = [util.a2hex(
	"http/1.1"), util.a2hex("spdy/1"), util.a2hex("spdy/2"), util.a2hex(
	    "spdy/3")];
...
const defaultClientHello = function (cfg:config, cr:string,
				     keys: {p256:bytes,ff2048:bytes}) {
    const ch : client_hello = {
...
function clientHelloBytes(ch:client_hello) : string{
    const data = ch.protocol_version + ch.client_random + vlbytes(1, ch.sessionID) +
	  vlbytes(2, cipherSuitesBytes(ch.cipher_suites)) + vlbytes(1,
								    compressionsBytes(ch.compressions)) +
	  extensionsBytes(ch.extensions, clientExtensionBytes);
    return messageBytes(HT.client_hello, data);
}

function parseClientHello(d:string) : result<client_hello>{
    if (util.getLength(d) < 34) return (Incorrect(AD.decode_error,
						  "clientHello less than 34 bytes"));
    else {
	const sp1 = util.split(d, 2);
	const pv = sp1.fst;
	const sp2 = util.split(sp1.snd, 32);
	const cr = sp2.fst;
	if (util.getLength(sp2.snd) < 1) return (Incorrect(AD.decode_error,
							   "clientHello: no session id"));
	else {
	    return bindError(vlsplit(1, sp2.snd),(sp3 => {
		if (util.getLength(sp3.fst) > 32) return (Incorrect(AD.decode_error,
								    "clientHello: session id more than 32 bytes"));
		else {
		    const sid = sp3.fst;
		    if (util.getLength(sp3.snd) < 2) return (Incorrect(AD.decode_error,
								       "clientHello: no ciphersuites"));
		    else {
			return bindError(vlsplit(2, sp3.snd),(sp4 => bindError(parseCipherSuites(sp4.fst),(
			    cs => {
				if (util.getLength(sp4.snd) < 1) return (Incorrect(AD.decode_error,
										   "clientHello: no compressions"));
				else {
				    return bindError(vlsplit(1, sp4.snd),(sp5 => bindError(parseCompressions(sp5.fst), (cm => mapError(parseExtensions(sp5.snd, parseClientExtension), (
					exts => ({
					    protocol_version: pv,
					    client_random: cr,
					    sessionID: sid,
...
const defaultServerHello = function (sr: string) : server_hello {
    const sh: server_hello = {
	protocol_version: PV.TLS_1p2,
	server_random: sr,
	sessionID: "",
...
function serverHelloBytes(sh:server_hello) {
    var data;
    console.log(sh);
	...
	console.log(exts);
    	data = sh.protocol_version + sh.server_random +
	    sh.cipher_suite + exts;
    }
    else
	data = sh.protocol_version + sh.server_random + vlbytes(1, sh.sessionID) +
	sh.cipher_suite + sh.compression + extensionsBytes(sh.extensions,
							   serverExtensionBytes);
    return messageBytes(HT.server_hello, data);
}

function parseServerHello(d:string) : result<server_hello> {
    if (util.getLength(d) < 34) return (Incorrect(AD.decode_error,
						  "serverHello: less than 34 bytes"));
    else {
	const sp1 = util.split(d, 2);
	const pv = sp1.fst;
	const sp2 = util.split(sp1.snd, 32);
	...
	var cm = "";
	if (util.getLength(sp2.snd) < 1) return (Incorrect(AD.decode_error,
							   "serverHello: no ciphersuite"));
	else {
	    var rem = sp2.snd;
	    if (pv != PV.TLS_1p3 && pv != "7f14") {
		const sp3 = vlsplit(1, sp2.snd);
		let res: result<pair<string,string>> = sp3;
		switch (res.result) {
		case "Error":
		    return (Incorrect(AD.decode_error,
				      "serverHello: no session id"));
		case "Correct":
		    if (util.getLength(res.value.fst) > 32)
			return (Incorrect(AD.decode_error,
					  "serverHello: session id too large"));
		    else {
			sid = res.value.fst;
			rem = res.value.snd;
			break;
		...
		return (Incorrect(AD.decode_error,
				  "severHello: no ciphersuite,compression"));
	    } else {
...
const defaultServerCertificate : certificate = {
    chain: [tls_crypto.server_cert.hex]
}

function certificateBytes(c:certificate, pv:string) {
    if (pv === PV.TLS_1p3) {
...
function parseCertificate(b:string, pv:string): result<certificate> {
    if (pv === PV.TLS_1p3) {
		...
		while (util.getLength(clb) > 0) {
		    ...
		    let sp2: result<pair<string,string>> = vlsplit(2,res.value.snd);
		    switch (sp2.result) {
		    case "Error":
...
const defaultCertificateRequest : certificate_request = {
    certificate_types: Object.keys(CT).map(k => CT[k]),
    signature_algorithms: Object.keys(SS).map(k => SS[k]),
    distinguished_names: []
}

function certificateRequestBytes(scr:certificate_request, pv:string) {
...
function parseCertificateRequest(b:string, pv:string): result<certificate_request> {
    return bindError(vlsplit(1, b), (sp => {
	const ctl = parseCertificateTypes(sp.fst);
	if (pv == PV.TLS_1p2) {
	    return bindError(vlsplit(2, sp.snd),(sp2 => bindError(parseSignatureAlgorithmsList(sp2.fst),
								  (sal => bindError(vlparse(2, sp2.snd),(dnb =>
													 mapError(parseDistinguishedNames(dnb),(dns => ({
													     certificate_types: ctl,
													     signature_algorithms: sal,
													     distinguished_names: dns
													 })))))))))
	} else return bindError(vlparse(2, sp.snd),(dnb => mapError(parseDistinguishedNames(dnb),
								    (dns => ({
									certificate_types: ctl,
...
const defaultServerKeyExchange_ECDHE = function (dh_public:string) : server_key_exchange {
    return {
	kex: "ECDHE",
	ec_params: {
	    curve: SG.secp256r1
	...
	sign: function (cr:string, sr:string, pv:string) {
	    const kexB = "03" + this.ec_params.curve + vlbytes(1, this.ec_public);
	    const sigv = cr + sr + kexB;
	    if (pv == PV.TLS_1p0) {
		this.sig.sig_value = tls_crypto.rsa_sign(
		    tls_crypto.server_key, tls_crypto.md5(sigv) + tls_crypto.sha1(sigv)
		);
	    } else {
...
const defaultServerKeyExchange_DHE : server_key_exchange = {
    kex: "DHE",
    dh_params: {
	dh_p: "00f488fd584e49dbcd20b49de49107366b336c380d451d0f7c88b31c" +
	    "7c5b2d8ef6f3c923c043f0a55b188d8ebb558cb85d38d334fd7c175743" +
	    "a31d186cde33212cb52aff3ce1b1294018118d7c84a70a72d686c40319" +
	    "c807297aca950cd9969fabd00a509b0246d3083d66a45d419f9c7cbd89" +
	    "4b221926baaba25ec355e92f78c7",
	dh_g: "02"
    },
    dh_public: "02",
    sig: {
    ...
    sign: function (cr:string, sr:string, pv:string) {
	const kexB = vlbytes(2, this.dh_params.dh_p) + vlbytes(2, this.dh_params
							       .dh_g) + vlbytes(2, this.dh_public);
	const sigv = cr + sr + kexB;
	if (pv == PV.TLS_1p0) this.sig.sig_value = tls_crypto.rsa_sign(
	    tls_crypto.server_key, tls_crypto.md5(sigv) + tls_crypto.sha1(sigv));
	else {
	    switch (this.sig.sig_hash_alg) {
	    case SS.rsa_pkcs1_sha256:
...
const defaultServerKeyExchange_RSA : server_key_exchange = {
    kex: "RSA",
    rsa_public: {
	rsa_modulus: "00f488fd584e49dbcd20b49de49107366b336c380d451d0f7c88b31c" +
	    "7c5b2d8ef6f3c923c043f0a55b188d8ebb558cb85d38d334fd7c175743a31d186" +
	    "cde33212cb52aff3ce1b1294018118d7c84a70a72d686c40319c807297aca950c" +
	    "d9969fabd00a509b0246d3083d66a45d419f9c7cbd894b221926baaba25ec355e" +
	    "92f78c7",
	rsa_exponent: "010001"
    },
    sig: {
	...
	const kexB = vlbytes(2, this.rsa_public.rsa_modulus) + vlbytes(2, this.rsa_public
								       .rsa_exponent);
	const sigv = cr + sr + kexB;
	if (pv == PV.TLS_1p0) this.sig.sig_value = tls_crypto.rsa_sign(
	    tls_crypto.server_key, tls_crypto.md5(sigv) + tls_crypto.sha1(sigv));
	    ...
	    case SS.rsa_pkcs1_md5:
		this.sig.sig_value = tls_crypto.rsa_md5(tls_crypto.server_key, (sigv)); break;
	    default: throw new Error("only RSA-SHA256/SHA1/MD5 signatures implemented")
	    }
	}
    }
}

function serverKeyExchangeParamsBytes(ske:server_key_exchange) {
    let kexB = "";
    switch (ske.kex) {
    case "DHE":
	kexB = vlbytes(2, ske.dh_params.dh_p) + vlbytes(2, ske.dh_params.dh_g) +
	    vlbytes(2, ske.dh_public);
	return kexB;
    case "ECDHE":
	kexB = "03" + ske.ec_params.curve + vlbytes(1, ske.ec_public);
	return kexB;
    case "RSA":
	kexB = vlbytes(2, ske.rsa_public.rsa_modulus) + vlbytes(2, ske.rsa_public
								.rsa_exponent);
	return kexB;
    default:
...
function serverKeyExchangeBytes(ske:server_key_exchange, pv:string) {
    let kexB = serverKeyExchangeParamsBytes(ske);
    let sig = "";
    if (pv == PV.TLS_1p2) sig = ske.sig.sig_hash_alg + vlbytes(2,ske.sig.sig_value);
    else sig = vlbytes(2, ske.sig.sig_value);
    return messageBytes(HT.server_key_exchange, kexB + sig);
}



function parseSignature(b:string, pv:string) : result<signature> {
    //    console.log("parseSignature version:"+pv+"="+PV.TLS_1p2);
    if (pv == PV.TLS_1p2 || pv == PV.TLS_1p3) {
	if (util.getLength(b) <= 2) return Incorrect(AD.decode_error,
						     "TLS 1.2 signature not long enough");
	else {
	    const sp1 = util.split(b, 2);
...
function parseServerKeyExchange(b:string, pv:string, kex:string) : result<server_key_exchange>{
    //    console.log("pv:"+pv+",kex:"+kex);
    switch (kex) {
    case "DHE":
	return bindError(vlsplit(2, b), (sp1 => bindError(vlsplit(2, sp1.snd),(sp2 => bindError(vlsplit(
	    2, sp2.snd),(sp3 => mapError(parseSignature(sp3.snd, pv),(sig => ({
		kex: "DHE",
		dh_params: {
		    dh_p: sp1.fst,
		    dh_g: sp2.fst
    ...
    case "ECDHE":
	if (util.getLength(b) < 4) return Incorrect(AD.decode_error,
						    "ECDHE ske not long enough");
	else {
	    const sp1 = util.split(b, 1);
	    const sp2 = util.split(sp1.snd, 2);
	    return bindError(vlsplit(1, sp2.snd),(sp3 => mapError(parseSignature(sp3.snd, pv),(
		sig => ({
		    kex: "ECDHE",
		    ec_params: {
			curve: sp2.fst
    ...
    case "RSA":
	return bindError(vlsplit(2, b),(sp1 => bindError(vlsplit(2, sp1.snd),(sp2 =>
									      mapError(parseSignature(sp2.snd, pv),(sig => ({
										  kex: "RSA",
										  rsa_public: {
										      rsa_modulus: sp1.fst,
...
const defaultClientKeyExchange_ECDHE = function (dh_public:string) : client_key_exchange {
    return {
	kex: "ECDHE",
	ec_public: dh_public
    }
...
function clientKeyExchangeBytes(cke:client_key_exchange, pv:string) {
    let kexB = "";
    switch (cke.kex) {
    case "DHE":
...
function parseClientKeyExchange(b:string, pv:string, kex:string): result<client_key_exchange> {
...
const defaultCertificateVerify = {
    ...
    signClient: function (log:string) {
	this.sig.sig_value = tls_crypto.rsa_sha256(tls_crypto.client_key, log)
    },
    signServer: function (log:string) {
	this.sig.sig_value = tls_crypto.rsa_sha256(tls_crypto.server_key, log)
    }
}

function certificateVerifyBytes(cv:certificate_verify, pv:string) {
    let sig = "";
    if (pv === PV.TLS_1p2 || pv === PV.TLS_1p3)
	sig = cv.sig.sig_hash_alg + vlbytes(2, cv.sig.sig_value);
    else sig = vlbytes(2, cv.sig.sig_value);
    return messageBytes(HT.certificate_verify, sig)
}

function parseCertificateVerify(b:string, pv:string) : result<certificate_verify> {
...
function finishedBytes(fin:finished) {
    return messageBytes(HT.finished, fin.verify_data)
}

function parseFinished(b:string) : result<finished>{
    return Correct({
	verify_data: b
    });
}
...
const defaultEncryptedExtensions : encrypted_extensions = {
    extensions: ""
}
...
function sessionTicketBytes(tick:new_session_ticket,pv: string) {
    if (pv === PV.TLS_1p3)
	return messageBytes(HT.session_ticket, "00000000" + "00000000" + vlbytes(2, tick.ticket) + vlbytes(2,""));
    else return messageBytes(HT.session_ticket, "00000000" + vlbytes(2, tick.ticket));
}

function parseEncryptedExtensions(b:string): result<encrypted_extensions> {
...
function parseSessionTicket(b:string,pv:string): result<new_session_ticket> {
	...
	let flags = sp2.fst;
	return bindError(vlsplit(2,sp2.snd),(sp =>
					     mapError(vlparse(2, sp.snd),(t => {
						 return ({
						     lifetime: lt,
						     flags: flags,
						     extensions: sp2.fst,
	...
	return mapError(vlparse(2, sp.snd),(t => {
	    return ({
		lifetime: lt,
		ticket: t
	    })}));
...
function parseHandshakeMessage(m:hs_msg, pv:string, kex:string) : result<any>{
    //console.log("phm - pv:"+pv+",kex:"+kex);
    //     console.log(m);
    if (typeof(m.pl) !== "string")
	throw ("Already parsed, payload not a string\n" + new Error().stack)
    ...
    case HT.certificate:
	return parseCertificate(m.pl, pv)
    case HT.server_key_exchange:
	return parseServerKeyExchange(m.pl, pv, kex)
    case HT.server_hello_done:
	if (util.getLength(m.pl) == 0) return Correct(util.empty_bytes);
	else return Incorrect(AD.decode_error, "serverHello done is not empty")
...
function ContentType_lookup(x:string) {
    return lookup(ContentType_rev, x)
};


function serializeRecord(r:record) {
    if (debug) console.log(r);
    return r.type + r.version + vlbytes(2, r.fragment);
}

function parseRecord(b:string): result<record> {
    if (util.getLength(b) < 5) return Incorrect(AD.decode_error,
						"parseRecord: record has less than 5 bytes");
    const sp1 = util.split(b, 1);
...
function splitRecord(b:string): result<pair<record,string>> {
    if (util.getLength(b) < 5) return Incorrect(AD.decode_error,
						"splitRecord: record has less than 5 bytes");
    const sp1 = util.split(b, 1);
    const sp2 = util.split(sp1.snd, 2);
