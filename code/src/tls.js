...
//	const typechecked = require('./tls_typechecked.js');
	...
	const Skip_CCS_server_callbacks = {
			...
			hs_recv_client_finished: function (msgs, cipherState) {
				let out_msgs = handshake.TLS_server_callbacks.hs_recv_client_finished(msgs,
					cipherState);
	...
	const Freak_server_callbacks = {
			hs_recv_client_hello: function (msgs, cipherState) {
				...
				if (ch.pl.protocol_version < formats.PV.TLS_1p0 || !util.mem(formats.CS.TLS_RSA_WITH_AES_128_CBC_SHA,
						ch.pl.cipher_suites)) {
					console.log("no TLS-RSA ciphersuite in client hello:" + pp.printLog(
						msgs));
					return [];
				}
				...
				out_msgs.push(handshake.handshakeRecord(cipherState, formats.serverHelloBytes(
					sh)));
				let sc = formats.defaultServerCertificate;
				out_msgs.push(handshake.handshakeRecord(cipherState, formats.certificateBytes(
				    sc,pv)));
				let ske = formats.defaultServerKeyExchange_RSA;
				ske.sign(cipherState.cr, cipherState.sr, cipherState.pv);
				out_msgs.push(handshake.handshakeRecord(cipherState, formats.serverKeyExchangeBytes(
					ske, cipherState.pv)));
	...
	const MD5_downgrade_server_callbacks = {
		hs_recv_client_hello: function (msgs, cipherState) {
			...
			if (ch.pl.protocol_version < formats.PV.TLS_1p2 || !util.mem(formats.CS.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA,
					ch.pl.cipher_suites)) {
				console.log("no TLS 1.2 ECDHE ciphersuite in client hello:" + pp.printLog(
					msgs));
				return [];
			}
			...
			out_msgs.push(handshake.handshakeRecord(cipherState, formats.certificateBytes(
			    sc,pv)));
			let ske = formats.defaultServerKeyExchange_ECDHE(
				cipherState.p256r1_keys.ec_public
			);
	...
	if (process.argv.length <= 3) {
		...
		if (process.argv[2] === 'server') {
		    if (process.argv.length == 3) record.TLS_server_scenario(4443,null_cb);
		    else record.TLS_server_scenario(parseInt(process.argv[3]),null_cb);
		} else if (process.argv[2] === 'client') {
		    if (process.argv.length == 3) record.TLS_client_scenario("localhost", 4443,null_cb);
			else {
			...
			let cb = function () {
			    if (process.argv.length == 3) record.TLS_client_scenario("localhost", 4443,null_cb);
			    else record.TLS_client_scenario(process.argv[3], parseInt(process.argv[4]),null_cb)
			}
			if (process.argv.length == 3) record.TLS_client_scenario("localhost", 4443, cb);
		    else record.TLS_client_scenario(process.argv[3], parseInt(process.argv[4]), cb);
		} else if (process.argv[2] === 'test-server') {
			if (process.argv.length < 4 || !util.mem(process.argv[3], ["freak", "skip-ccs",
					"md5-sig"
				])) {
				usage();
				return false
	...
	return {
	    get: function(uri:string, cb:(string=>void)) {
		record.TLS_client_scenario(uri, 443, function(res:string) {
